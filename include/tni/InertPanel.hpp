#ifndef TNI_API_HEADER_INERTPANEL
#define TNI_API_HEADER_INERTPANEL
...
struct InertPanel : public RigidBody2D {
	using RigidBody2D::RigidBody2D;

	constexpr InertPanel(RigidBody2D base) : RigidBody2D{base} {}
	constexpr InertPanel(uint64_t addr) : RigidBody2D{addr} {}
	constexpr InertPanel(Object obj) : InertPanel{obj.address()} {}
	InertPanel(Variant variant) : InertPanel{variant.as_object().address()} {}
...
inline void InertPanel::apply_autoconfig() { voidcall("apply_autoconfig"); }
inline void InertPanel::reposition(Variant new_pos) { voidcall("reposition", new_pos); }
inline void InertPanel::elevator_move(Variant new_pos) { voidcall("elevator_move", new_pos); }
inline double InertPanel::get_device_bounding_height() { return operator()("get_device_bounding_height"); }
inline Variant InertPanel::get_global_y_range() { return operator()("get_global_y_range"); }
inline Variant InertPanel::get_local_y_range() { return operator()("get_local_y_range"); }
inline Variant InertPanel::debug_monitor_callback() { return operator()("debug_monitor_callback"); }
inline Variant InertPanel::debug_mux_setup() { return operator()("debug_mux_setup"); }
inline Variant InertPanel::update_in_trolley_state() { return operator()("update_in_trolley_state"); }
inline bool InertPanel::pickup(Variant new_picker) { return operator()("pickup", new_picker); }
inline bool InertPanel::drop(Variant impulse) { return operator()("drop", impulse); }
inline void InertPanel::reset_child_z_index() { voidcall("reset_child_z_index"); }
inline void InertPanel::set_autosvc(bool new_state) { voidcall("set_autosvc", new_state); }
inline void InertPanel::update_user_note(String new_value) { voidcall("update_user_note", new_value); }
inline void InertPanel::remove_and_free_object() { voidcall("remove_and_free_object"); }
inline void InertPanel::lift_child_z_index(int64_t base_val) { voidcall("lift_child_z_index", base_val); }
inline Variant InertPanel::get_picker_type(Variant test_picker) { return operator()("get_picker_type", test_picker); }
inline void InertPanel::setup_teleport(Variant gpos) { voidcall("setup_teleport", gpos); }
